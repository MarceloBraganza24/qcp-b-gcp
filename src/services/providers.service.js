...
import { ProviderByEmailExists, ProviderByCuitCuilExists, ProviderExists } from '../utils/custom.exceptions.js';
...
const update = async (id, provider) => {
    ...
    if(providerById.business_name !== provider.business_name || providerById.cuit_cuil !== provider.cuit_cuil || providerById.phone !== provider.phone || providerById.email !== provider.email) {
        if(providerByCuitCuilExists.business_name !== provider.business_name && providerByCuitCuilExists.phone !== provider.phone && providerByCuitCuilExists.email !== provider.email) {
            throw new ProviderByCuitCuilExists('There is already a provider with that CUIT-CUIL');
        }
        if(providerByEmailExists.business_name !== provider.business_name && providerByEmailExists.cuit_cuil !== provider.cuit_cuil && providerByEmailExists.phone !== provider.phone) {
            throw new ProviderByEmailExists('There is already a provider with that email');
        }
